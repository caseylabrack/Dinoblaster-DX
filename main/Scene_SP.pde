...
class SinglePlayer extends Scene {
  ...
  void render () {
    ...
    ui.render();
    imageMode(CORNER);
    if (!settings.getBoolean("hideHelpButton", false)) image(assets.uiStuff.optionsBtn, optionsButton.x, optionsButton.y, assets.uiStuff.optionsBtn.width/2, assets.uiStuff.optionsBtn.height/2);
    popStyle();
    popMatrix();
  ...
  void mouseUp() {
    ...
    if (optionsButton.inside(m) && settings.getBoolean("hideHelpButton", false)==false) {
      options = !options;
    }
